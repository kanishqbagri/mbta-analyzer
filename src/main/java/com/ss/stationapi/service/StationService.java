...
public class StationService {
    ...
    //API to fetch the Name, Latitude and Longitude of the Stations
    public static Map<String, StationInfo> getStationInfo() {
        ...
        try {
            ...
            for (JsonNode node : root) {
                ...
                System.out.println("Station Name: " + station.getId() + " " + station.getLongitude());
                stationById.put(station.getName(), station);
            }
...
//    private static void getLineInfoForEachStation(){
//        for(Map.Entry<String, StationInfo> entry : stationById.entrySet()){
//            String stationId = entry.getKey();
//            StationInfo station = entry.getValue();
//
//            Set<String> lineInfo = linesByStation(stationId);
//            station.addLine(lineInfo);
//        }
//    }
    ...
    private static Map<String, StationInfo> getAdjacentStops1(String route) {
        ...
        try {
            String url = BASE_URL + "schedules?include=stop&sort=stop_sequence&filter[route]=" + route;
            ...
            for (int i = 0; i < stopIds.size(); i++) {
                String currentStop = stopIds.get(i);
                adjacencyMap.putIfAbsent(currentStop, new HashSet<>());

                if (i > 0) {
                    String prev = stopIds.get(i - 1);
                    adjacencyMap.get(currentStop).add(prev);
                    adjacencyMap.get(prev).add(currentStop);
                }
            }
        }
        catch (Exception e){
