...
@Component
public class StationService_v1 {
...
//        linesByStation(70159); // Fetch Line info for the station
        ...
        getStationInfo_v2().values().forEach(s -> System.out.println(s.getName()));
        //        getAdjacentStops();
    }
    ...
    private static void loadStationsAndRoutes() {
        try {
            // Fetch all routes and map stops to lines
    ...
    private static void loadAdjacency() {
        try {
            for (String lineId : lines) {
                String url = BASE_URL + "schedules?filter[route]=" + lineId + "&filter[direction_id]=0&sort=stop_sequence";
                HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).build();
    ...
    public static Map<String, StationInfo> getStationInfo_v2() {
        return stationById;
    }

    // Only returns cached data, does not fetch from MBTA
    public static Set<String> linesByStation_v2(String stopId) {
    ...
    private static Map<String, Set<String>> fetchAllRoutes() throws IOException, InterruptedException {
        String url = BASE_URL + "routes?filter[type]=0,1";
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).build();
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        JsonNode root = mapper.readTree(response.body()).get("data");
